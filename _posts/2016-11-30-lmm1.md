...
## Obtain the restricted maximum likelihood (REML) estimates of $\sigma^2$,$\tau^2$.

## Also obtain the REML estimates of the ﬁxed eﬀects $\hat{\beta}$.
...
## Explain how the standard errors are obtained for the REML
...
-   We know that
    ...
    *V* = *V*(*\theta*)=*\sigma*<sup>2</sup> \* *Z* \* *Z*′+*\tau*<sup>2</sup> \* *I*<sub>*n*</sub>
...
-   Last we take the diagonals of $\hat{S}$ and the square root of that
    ...
    *\beta*<sub>*j*</sub>'s standard error is the *j*<sup>*t**h*</sup>
...
-   PB(Parametric Bootstrap) method
    ...
    -   For b in 1:B (repeat the folowing two steps for B times)
        ...
        2.  Fit a Linear Mixed Model to that sample to find REML
            estimates of *\theta* = (*\sigma*<sup>2</sup>, *\tau*<sup>2</sup>). Let
            $\hat{\theta}^{(b)}\_{\sigma^{2}}$ and
            $\hat{\theta}^{(b)}\_{\tau^{2}}$ be the REML estimates of
            *\sigma*<sup>2</sup> and *\tau*<sup>2</sup> respectively from
            the sample.
...
-   PB method
    -   replace the *\sigma*<sub>*R**E**M**L*</sub><sup>2</sup> and
        *\tau*<sub>*R**E**M**L*</sub><sup>2</sup> with
        *\sigma*<sub>*M**L*</sub><sup>2</sup> and
        *\tau*<sub>*M**L*</sub><sup>2</sup>
    -   The standard errors for the ML estimates of the variance
        components by PB method are
