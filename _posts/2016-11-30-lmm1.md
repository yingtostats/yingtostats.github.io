...
### The Restricted Maximum Likelihood (REML) Estimates of $\sigma^2$,$\tau^2$, and the ﬁxed eﬀects $\beta$.
...
### How the standard errors are obtained for the REML estimates of the ﬁxed eﬀects?
...
### ACM and PB methods for obtaining the standard errors for the variance-component estimates.
...
-   PB(Parametric Bootstrap) method
    ...
    -   For b in 1:B (repeat the following two steps for B times)
        ...
        2.  Fit a Linear Mixed Model to that sample to find REML
            estimates of $$\theta = (\sigma^{2}, \tau^{2})$$. Let
            $\hat{\theta}^{(b)}\_{\sigma^{2}}$ and
            $\hat{\theta}^{(b)}\_{\tau^{2}}$ be the REML estimates of
            $\sigma^{2}$ and $\tau^{2}$ respectively from
            the sample.
...
#### Compute the standard errors of REML estimates via the asymptotic covariance matrix (ACM) method.
...
------------------------------------------------------------------------
#### Compute the standard errors in REML estimates via the parametric bootstrap (PB) method.
...
#### Compare the results of ACM and PB Comment on what you observe.
...
#### Repeat previous parts using MLE instead of REML estimates.
