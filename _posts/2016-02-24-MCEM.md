...
I(\theta) \approx I_{OC}(\theta)
          = - E[\frac{\partial^{2}}{\partial \theta \partial \theta '}log\*f\*(y, \alpha \mid \theta)\mid y, \theta]
$$
$$= - E[\frac{\partial^{2}}{\partial \theta \partial \theta '}log\*f\*(y \mid \alpha \theta)\mid y, \theta ] - E[\frac{\partial^{2}}{\partial \theta \partial \theta '}log\*f\*(\alpha \mid \theta)\mid y, \theta ]
$$
$$\approx C_{MH} + D_{MH} $$
$$\approx \hat{C}_{MH} + \hat{D}_{MH} $$

where $C_{MH}$ and $D_{MH}$ are Monte Carlo approximations(via Metropolis-Hastings algorithm) of C and D, respectively, using the last set of samples $\alpha^{(d)}_{i} \mid y_{i}, \theta^{K-1}$'s, $i = 1,...,m$, $d = 1,...,D$, where K is the last iteration in the MCEM algorithm. Then $\hat{C}_{MH} =  C_{MH}\mid _{\theta = \hat{\theta}}$ and  $\hat{D}_{MH} =  D_{MH}\mid _{\theta = \hat{\theta}}$
