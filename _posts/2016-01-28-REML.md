...
$$I^{-1}(\hat{\theta}_{REML}) = \frac{1}{2}\Big( \begin{matrix}
  \text{tr}(\hat{P}ZZ'\hat{P}ZZ') & \text{tr}(\hat{P}^{2}ZZ') \\
  \text{tr}(\hat{P}^2ZZ') & \text{tr}(\hat{P}^{2})
\end{matrix} \Big) ^{-1}$$
*  Then we can obtain
{%latexblock%} I^{-1}(\hat{\theta}_{REML}) =
\Big( \begin{matrix}
  a^{\ast} & b^{\ast} \\
  c^{\ast} & d^{\ast}
\end{matrix} Big) {%endlatexblock%}
...
*  The standard errors of $$\hat{\sigma}^{2}_{\text{REML}}$$ are much more
higher than that of $$\hat{\tau}^{2}_{\text{REML}}$$ for both methods.
...
$$ I^{-1}(\hat{\theta}_{ML}) = \frac{1}{2}\Big( \begin{matrix}
