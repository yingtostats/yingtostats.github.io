...
*  The REML estimate of the inverse of the restricted Fisher
  ...
  $$I^{-1}(\hat{\theta}_{REML}) = \frac{1}{2}\begin{matrix}
  tr(\hat{P}ZZ'\hat{P}ZZ') & tr(\hat{P}^{2}ZZ') \\
  tr(\hat{P}^2ZZ') & tr(\hat{P}^{2})\end{matrix}^{-1} $$
*  Then we can obtain
  $$I^{-1}(\hat{\theta}_{REML}) =
  \begin{matrix}
  a^{*} & b^{*} \\
...
*  Replace the $\hat{P}$ with $\hat{V}^{-1}$ in
  ...
  $$I^{-1}(\hat{\theta}_{ML}) = \frac{1}{2}\begin{matrix}
  tr(\hat{V}^{-1}ZZ'\hat{V}^{-1}ZZ') & tr(\hat{V}^{-2}ZZ') \\
  tr(\hat{V}^{-2}ZZ') & tr(\hat{V}^{-2})\end{matrix}^{-1} $$
*  Then we can obtain
  $$I^{-1}(\hat{\theta}_{ML}) =
  \begin{matrix}
  a^{*} & b^{*} \\
